...
#| message: false
#| label: fig-sex-plots
#| fig-cap: Sex distribution of wild and captive lemurs represented by bar plots for female (F) and male (M) individuals.
# Display plots
...
#| fig-cap: Distribution of lemurs by the month they were born in. On left is the distribution for lemurs in the wild and on the right is for lemurs in captivity.
...
#| fig-cap: Distribution of ages of lemurs in the dataset for lemurs in captivity (on right) and in the wild (on left).
...
@sec-appendix-model covers coefficients, standard errors, convergence checks and other model diagnostics of our model. 
